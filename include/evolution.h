...
namespace lmu
{
	...
	template<typename RankedCreature>
	struct FitnessReachedStopCriterion
	{
		FitnessReachedStopCriterion(double value, double delta, int maxIterations) :
			_value(value),
			_delta(delta),
			_maxIterations(maxIterations)
		{
		...
		bool shouldStop(const std::vector<RankedCreature>& population, int iterationCount)
		{
			...
			double curBestValue = population[0].rank;

			if (std::abs(curBestValue - _value) <= _delta)
				return true;
		...
		{
			std::stringstream ss;
			ss << "Fitness Reached Stop Criterion (value=" << _value << ", delta=" << _delta << ", " << _maxIterations << ")";
			return ss.str();
		}
